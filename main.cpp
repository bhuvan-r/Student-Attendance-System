...
int home() {
  ...
  SetColor(45);
  cout<<"\n\n";
  cout<<"\t\t\t\t\t ==> 1. Log In as Teacher\n";
  cout<<"\t\t\t\t\t ==> 2. Log In as Admin\n";
  cout<<"\t\t\t\t\t ==> 3. CGPA Calculator\n";
  cout<<"\t\t\t\t\t ==> 4. EXIT Software\n\n";
  SetColor(11);
  cout<<"\t\t\t\t\t Select an Option:";
  SetColor(12);
  gotoxy(70, 25);
  cout<<"© Developed By TEAM YO";
  SetColor(45);
  gotoxy(59, 14);
  hswitch = getche();
  switch (hswitch) {
...
int attendance_data() {
  ...
  if (found == 0) {
    while (fread( &stdd, sizeof(stdd), 1, fptr1)) {
      fflush(stdin);
      strcpy(s_att.date, date);
      strcpy(s_att.sid, stdd.id);
      strcpy(s_att.sname, stdd.name);
      SetColor(13);
...
int save_student_details() {
  ...
  printf("\n   \tEnter Average CGPA: ");
  SetColor(11);
  scanf("%f", & stdd.Av_cgpa);
  SetColor(14);

  fwrite( & stdd, sizeof(stdd), 1, fptr);
  printf("\n\n >> SAVE SUCCESSFUL <<\n");
  fclose(fptr);
...
csv.open("test.csv");
  ...
  fpsr = fopen("Attendance.dll", "r");
  while (fread( & s_att, sizeof(s_att), 1, fpsr)) {
{     csv<<"Student name, Student ID, Student Attandance"<<endl; 
...
int view_student_details() {
  ...
  while (fread( & stdd, sizeof(stdd), 1, fptr1)) {
    SetColor(11);
    printf("\n\t\t\t  NAME: %s\n\t\t\t  ID: %s\n\t\t\t  Section: %s\n\t\t\t  Batch: %s\n\t\t\t  Address: %s\n\t\t\t  Department: %s\n\t\t\t  Semester: %s\n\t\t\t  AVERAGE CGPA: %.2f\n", stdd.name, stdd.id, stdd.section, stdd.batch, stdd.address, stdd.department, stdd.semester, stdd.Av_cgpa);
  }
  fclose(fptr1);
...
int search_student() {
  ...
  while (fread( & stdd, sizeof(stdd), 1, fptdt)) {
    tdt = tdt + 1;
  }
  fclose(fptdt);
  search1(tdt);
...
int search1(int tdt) {
  ...
  while (fread( & stdd, sizeof(stdd), 1, fptr2)) {
    if (strcmp(id, stdd.id) == 0) {
      printf("\n\n\t\t\t  Student Information\n");
      printf("\n\n\t\t\t  NAME: %s\n\t\t\t  ID: %s\n\t\t\t  Section: %s\n\t\t\t  Batch: %s\n\t\t\t  Address: %s\n\t\t\t  Department: %s\n\t\t\t  Semester: %s\n\t\t\t  AVERAGE CGPA: %.2f\n", stdd.name, stdd.id, stdd.section, stdd.batch, stdd.address, stdd.department, stdd.semester, stdd.Av_cgpa);
      break;
    }
...
int search_student_semester1(int tdt) {
  ...
  while (fread( & stdd, sizeof(stdd), 1, fptr2)) {
    if (strcmp(smstr, stdd.semester) == 0) {
      printf("\n  Search Matching\n");
      printf("\n  NAME: %s\n  ID: %s\n  Section: %s\n  Batch: %s\n  Address: %s\n  Department: %s\n  Semester: %s\n  AVERAGE CGPA: %.2f\n", stdd.name, stdd.id, stdd.section, stdd.batch, stdd.address, stdd.department, stdd.semester, stdd.Av_cgpa);
      for (stdd.i = 1; stdd.i <= stdd.n_sub; stdd.i++) {
        printf("  Subject: %d : %s GPA: %.2f\n", stdd.i, stdd.sub[stdd.i].sub_name, stdd.sub[stdd.i].cgpa);
      }
      break;
    }
    lc = lc + 1;
...
int edit_student() {
  ...
  while (fread( & stdd, sizeof(stdd), 1, edit)) {
    td = td + 1;
  }
  fclose(edit);
  edit1(td);
...
int edit1(int td) {
  ...
  struct student stdd, std1, std3, std4;
...
int search_student_attendance() {
...

  {
    ...
    while (fread( & stdd, sizeof(stdd), 1, stdptr1)) {
      strcpy(idfc, stdd.id);
      break;
    }
    fclose(stdptr1);
    ...
    while (fread( & stdd, sizeof(stdd), 1, stdptr)) {
      if (strcmp(id, stdd.id) == 0) {
        SetColor(11);
        gotoxy(50, 7);
        printf(">> Attendance <<\n\n");
        SetColor(14);
        gotoxy(50, 9);
        printf("Name:%s", stdd.name);
        gotoxy(50, 10);
        printf("ID: %s", stdd.id);
        gotoxy(50, 11);
        printf("Department: %s", stdd.department);
        gotoxy(50, 12);
        printf("Section: %s", stdd.section);
        gotoxy(50, 14);
    ...
    mark_att1 = (present*100) / count1;

    gotoxy(50, 20);
    printf("Attendance Percentage: %0.2f  \n", mark_att1);
    gotoxy(50, 21);
...
int view_student_att() {
  ...
  while (fread( & stdd, sizeof(stdd), 1, fptr_att)) {
    SetColor(11);
    printf("\n\t  NAME: %s\n\t  ID: %s\n", stdd.name, stdd.id, stdd.section);
  }
  fclose(fptr_att);
